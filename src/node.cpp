...
void Node::spinLoop() {
  while (spinning_) {
      ...
      // 处理回调函数
      if (!callbacks_.empty()) {
          auto callbacks = std::move(callbacks_);  // 移动避免持有锁时执行回调
          callback_mutex_.unlock();  // 提前解锁，允许添加新回调
          
          for (const auto& callback : callbacks) {
              callback();
          }
          
          callback_mutex_.lock();  // 重新加锁继续循环
      }
      
      // 等待新事件或超时
      if (callbacks_.empty() && timers_.empty()) {
          // 没有回调或定时器，等待通知
...
void Node::printRegistry() {
  shm_manager_->printRegistry();
}