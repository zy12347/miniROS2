...
void Node::signalHandler(int signum) {
    if (signal_handler_node_) {
        ...
        if (signum == SIGTSTP || signum == SIGINT) {
            std::cout << "Signal received, exiting normally to allow cleanup..." << std::endl;
            // 不调用 exit(0)，让程序正常退出以便析构函数清理共享内存
            // 主循环应该检查 spinning_ 标志并退出
        }
    }
}
...
void Node::spinLoop() {
    ...
    while (spinning_) {
        ...
        // 注意：pthread_cond_wait() 会在等待时自动释放共享内存锁，被唤醒时重新获取锁
        ...
        uint64_t timeout       = std::min(min_timer_period_, static_cast<uint64_t>(100));   // 最多等待100ms
        ...
        // 3. 获取 registry_mutex_ 来访问本地 topics_ 数据（用于查找 event_id 对应的 topic）
        shm_manager_->shmManagerLockRegistry();
        int trigger_event_int =
                static_cast<int>(trigger_event);   // 在持有锁的情况下，收集需要处理的订阅者索引和对应的 event_id
        if (trigger_event_int > 0) {
            std::cout << "getTriggerEvent: " << trigger_event_int << " (binary: " << std::bitset<32>(trigger_event_int)
                      << ")" << std::endl;
            std::vector<int> processed_event_ids;   // 记录已处理的事件ID
            {
                std::lock_guard<std::mutex> lock(node_mutex_);   // 保护 subscriptions_ 访问
                std::cout << "Checking " << subscriptions_.size() << " subscriptions" << std::endl;
                for (size_t id = 0; id < subscriptions_.size() && id < subscription_event_ids_.size(); id++) {
                    int event_id = subscription_event_ids_[id];
                    std::cout << "  subscription[" << id << "] event_id: " << event_id << std::endl;
                    if (event_id >= 0 && event_id < 32) {   // 假设使用 32 位整数
                        // 检查对应的位是否被设置
                        bool is_triggered = (trigger_event_int & (1 << event_id)) != 0;
                        std::cout << "    bit " << event_id << " is " << (is_triggered ? "SET" : "NOT SET")
                                  << std::endl;
                        if (is_triggered) {
                            ...
                            try {
                                if (thread_pool_ && spinning_) {
                                    ...
                                    std::cout << "    Enqueued task for subscription[" << id << "]" << std::endl;
                                    // 记录已处理的事件ID
        ...
        for (auto& timer : timers_) {
            if (timer->isReady()) {
                ...
                try {
                    if (thread_pool_ && spinning_) {
                        // 使用捕获的 shared_ptr，不需要访问 Node 的成员
                        std::function<void()> task_func = timer->createTaskFromTimer();   // 拷贝数据并创建任务
                        thread_pool_->enqueue(std::move(task_func));
...
// 停止Spin循环
void Node::stop() {
    std::cout << "Node::stop() called, setting spinning_ = false" << std::endl;
    spinning_ = false;
    ...
    std::cout << "Node::stop() waiting for spin_thread_ to join..." << std::endl;
    if (spin_thread_.joinable()) {
        spin_thread_.join();
    }
